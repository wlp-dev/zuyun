...
function getUserPic($sex){
    if ($sex==1) {
        return '/Application/event/Static/tree/imgs/man_middle_avatar.jpg';
    }else{
        return '/Application/event/Static/tree/imgs/woman_middle_avatar.jpg';
    }    
}


function getImg($id){
    $pic=D('picture')->find($id);
    return $pic['path'];   
}


function getUserIcon($sex){
    if ($sex==1) {
        return '/public/images/male.png';
    }else{
        return '/public/images/woman.png';
    }    
}
...
function get_spouse($uid){
    $p =  M("family_tree")->where(array('partner_id' => $uid))->order('partner_order')->select();
    foreach ($p as $value) {
        $spouse = $spouse.' 《 '.$value['name'] .' 》 ';
    }
    return $spouse;
}

function ebook_spouse($uid){
    $p =  M("family_tree")->where(array('partner_id' => $uid))->order('partner_order')->select();
    return $p;
}
 ...
 * @param $keys
 * @return mixed
 * @author :  xjw129xjt（駿濤） xjt@testzy.com
 */
