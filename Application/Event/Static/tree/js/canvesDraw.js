...
function init(){
   ...
   $('#save').click(function(){
      ...
      $.ajax({
         url : "index.php?s=/event/index/addUser",
...
function getInfoForUser(uid) {
   $.ajax({
      url : 'index.php?s=/event/index/ajaxPerson',
...
function addMember(data,isF){
   ...
   $('#addBox .goosip-btn').bind('click',function(){
      ...
      if($(this).attr('sex') == 'male'){
         $('#header').attr('src',w_baseImgUrl + '/Application/event/Static/tree/imgs/man.jpg');
      }else{
         $('#header').attr('src',w_baseImgUrl + '/Application/event/Static/tree/imgs/woman.jpg');
      }
...
function deletePerson(data){
   $.ajax({
         type : "post",
         url : "index.php?s=/event/index/delUser",
         dataType : "json",
         data : {
            user_id: data.userId,
...
function searchPerson(name){
    if(name) {
        $.ajax({
            url: "index.php?s=/event/index/searchPerson",
            type: 'post',
            dataType: 'json',
            data: {
                name: name,
            ...
            success: function (json) {
                ...
                if(json.length > 0){
                    ...
                    $.each(json, function (i, info) {
                       ...
                       select_html += '<div class="oh"><h3>' + info.name + '</h3> <span>' + info.sex + '</span><p>字辈：' + info.gname + '</p></div></li>';
...
function getData(fId,uId,x,y,t){
   $.ajax({
    url:"index.php?s=/event/index/member",
    type: 'post',
    dataType: 'json',
    data: {
      show_mod: w_showMod,
