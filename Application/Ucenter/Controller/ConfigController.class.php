...
1600:Lv7 '.L('_CHAIRMAN__').'
    ...
    public function other()
    {
        ...
        if ($sTime) {
           $map['sTime'] = array('like', '%'.$sTime.'%');
        }
        if ($eTime) {
            $map['eTime'] = array('like', '%'.$eTime.'%');
        }
    
        
        $content = D('Event')->where($map)->order($order)->limit(10)->select();
    ...
    public function family()
    {
        ...
        $map['uid'] = is_login();
        $kin = D('family_tree')->where($map)->find();
        if ($kin['family_id']) {

            $map2['id'] = $kin['family_id'];
            $content = D('Event')->where($map2)->select();
        }else{
            $content = array();
        }
        foreach ($content as &$v) {
    ...
    {
        $map['uid'] = is_login();
        $userMap['vendor_id'] = $map['uid'];
        $userList = D('member')->where($userMap)->select(); 
        foreach ($userList as $key => $value) {
            $arrMap[] = $value['uid'];
        }
        if (count($arrMap)>0) {
            $eventMap['uid']=array('in',$arrMap);
            $eventMap['type_id']=1;
            $eventMap['status']=1;
            $content = D('Event')->where($eventMap)->select();
        }else{
            $content = array();
        }
        foreach ($content as &$v) {
    ...
    {
        $uid = is_login();
        $invCode = base64_decode(I('post.invCode'));
        if ($uid && $invCode) {

            $data['id'] = $invCode;
            $data['uid'] = $uid;
            
            if (M("family_tree")->save($data)) {
                $this->success(L('_SUCCESS_CLEAR_').L('_EXCLAMATION_'), U('ucenter/config/family'));
            }
