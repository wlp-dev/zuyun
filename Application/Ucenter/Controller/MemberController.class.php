...
class MemberController extends Controller
{
    ...
    /**
     * register  注册页面
     * @author:xjw129xjt(肖骏涛) xjt@testzy.com
     */
    ...
    public function mi($email = '', $verify = '')
    {
        ...
        if (IS_POST) { //登录验证
            ...
            $content = C('USER_RESPASS') . "<br/>" . $url . "<br/>" . modC('WEB_SITE_NAME', L('_WPYCMS_'), 'Config') . L('_SEND_MAIL_AUTO_') . "<br/>" . date('Y-m-d H:i:s', TIME()) . "</p>";
            send_mail($email, modC('WEB_SITE_NAME', L('_WPYCMS_'), 'Config') . L('_SEND_MAIL_PASSWORD_FOUND_'), $content);
    ...
    public function doSendVerify($account, $verify, $type)
    {
        switch ($type) {
            case 'mobile':
                ...
                $content=array('product'=>modC('WEB_SITE_NAME',L('_WPYCMS_'),'Config'),'code'=>$verify);
            ...
            case 'email':
                ...
                $content = str_replace('{$verify}', $verify, $content);
                $content = str_replace('{$account}', $account, $content);
                $res = send_mail($account, modC('WEB_SITE_NAME', L('_WPYCMS_'), 'Config') . L('_EMAIL_VERIFY_2_'), $content);
                return $res;
                break;
        }
    ...
    private function sendActivateEmail($account, $verify, $uid)
    {
        ...
        $content = str_replace('{$title}', modC('WEB_SITE_NAME', L('_WPYCMS_'), 'Config'), $content);
        $res = send_mail($account, modC('WEB_SITE_NAME', L('_WPYCMS_'), 'Config') . L('_VERIFY_LETTER_'), $content);
...
//                    $this->error(L('_ERROR_EMAIL_LENGTH_LIMIT_'));
    ...
    /**
     * 自动生成邀请码
     * @author 路飞<lf@testzy.com>
     */
    public function createCode()
    {
        $invite_type = M('invite_type')->where(array('title' => '个人推荐码', 'create_time' => '1466749163'))->find();
